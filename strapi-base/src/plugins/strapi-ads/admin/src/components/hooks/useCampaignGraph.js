...
const useCampaignGraph = ({ page = 1, pageSize = 7, id, startDate, endDate }) => {
  ...
  const { data, error, isLoading, mutate } = useSWR(
    ['campaign-graph', id, startDate, endDate],
    () => get(`/${pluginId}/campaign/graph?${query}`)
  );

  return {
    campaignGraph: data?.data?.results ? [...data?.data?.results].reverse() : [],
    isLoading,
    isError: !!error,
    error,
    mutate,
